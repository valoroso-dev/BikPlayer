...
static const AVOption ffp_context_options[] = {
    ...
    { "live-quick-start",          "live quick start",
        OPTION_OFFSET(live_quick_start),      OPTION_INT(0, 0, 1) },
    { "av-sync-type",                    "av sync type, 0 by audio, 1 by video, 2 extrenal",
        OPTION_OFFSET(av_sync_type),     OPTION_INT(0, 0, 2) },
    { "fast-check-buffering",           "fast check buffering state after seek, 0, disable, 1 enable",
        OPTION_OFFSET(fast_check_buffering),     OPTION_INT(1, 0, 1) },
    { "is-drm-source",                    "0 not drm source, 1 drm source",
        OPTION_OFFSET(is_drm_source),     OPTION_INT(0, 0, 1) },
    {"index-drm-first",                     "use drm info parsed from mpd file first",
        OPTION_OFFSET(index_drm_first),     OPTION_INT(0, 0, 1) },
    {"enable-ijkplaylist",                  "enable ijkplaylist demuxer",
        OPTION_OFFSET(enable_ijkplaylist),  OPTION_INT(0, 0, 1) },
    {"check-no-pkt-recv",                  "check if no AVPacket received for a long time",
        OPTION_OFFSET(check_no_pkt_recv),   OPTION_INT(5, 0, INT_MAX) },
